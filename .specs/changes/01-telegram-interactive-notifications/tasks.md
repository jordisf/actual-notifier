...
- **S4 — DONE (dry-run e2e)** (2026-09-20, branch `slice/01-s4-listener-e2e`): `7b0e34a` `feat: run long-poll listener with categorize action executor` (+516: listener.js, actions/categorize.js + registry, dev/replay-callback.js). T0 gate reused from S1 (`updateTransaction(id, { category: categoryId })`). Dev-station e2e (D11, dev budget, dev-only): **unknown ref** (`--item-ref=3 --ref=99`) → `logAnswer rejected (unknown ref 99)` + status rolled back to `pending` ✅; **dismiss** (item 4) → `applied (dismissed)`, no Actual call, winner = first replayer ✅; **second tap on answered** → `duplicate (already answered)`, winner preserved ✅; **expired** (item 1 from report 1) → `expired (report expired)`, no state change ✅; **race attempt**: two concurrent replays on item 3 → one job applied (`applied` + real write-back), the other job hit a host path-resolution failure (Start-Job doesn't inherit cwd), so TRUE concurrent double-tap was not reproduced this time; idempotency is instead evidenced by the answered-guard (answer row 5: `duplicate (already answered)` on item 3's second touch) + the S1-verified guarded `UPDATE … WHERE status='pending'` first-wins semantics ✅; **Real write-back verified**: dev Actual tx "Nómina empresa" (+195, Cuenta Nómina) confirmed with category `Suscripciones (497d9e1f-…)` via API read; **then cleaned** (category null'd back, seed-invariant restored) ✅. Listener loop smoke: offset persists across restart (poll_offset kv), 404 backoff 1s→2s→4s, SIGTERM clean shutdown exit 0 ✅. `notifier-listener-dev` boots, detects no token, exits 1 cleanly (expected per §6.1) ✅. Deviation: el subagente escritor dejó `getDb()`/`setDb` usados sin definir en listener.js (ReferenceError en runtime, no en require); al verificar se añadieron los dos handlers a nivel de módulo en `listener.js` antes del commit (sin cambiar store.js). Live-tap steps (§12 step 6: @BotFather token, real group) NOT executed — no token available this session; blocked on user action, all othe [truncated — see full line]

**S4 live e2e (2026-09-20, real BotFather token, dev budget, user group)**: `@finjb_bot` → group; listener `notifier_listener-dev` live; report 3 delivered → real summary + 2 interactive messages (tg_message_id 4, 5; chats stored as real group id). Tap attempt 1: user tapped "Suscripciones" → "Desconocido (el botón ya no es válido)". Root cause (diagnosed via listener logs): the **diagnostician's manual `getUpdates` calls collided** with the listener's long-poll (HTTP 409 "terminated by other getUpdates request") at tap time; listener self-recovered; **not a code bug**. Standing rule: NO manual getUpdates while the live listener runs. Tap attempt 2 pending. **Note**: report 3 store row `telegram_tx_sent=1` vs 2 messages actually sent — counter to check in verify.

**S4 live e2e RESULT (2026-09-20, supergroup FinJBDMO `@finjb_bot`)**: Root causes of the first failed taps: (1) bot privacy mode — bot had to become group admin after Telegram's group→supergroup migration; (2) **split-brain SQLite (INFRA bug, not code)**: the long-lived `actual_notifier_dev` container predated the `data` volume mount in dev.yml and was never recreated, so cron delivered against its own ephemeral SQLite while the listener read the host-volume one → every tap looked unknown. Fixed by recreating containers, resetting the dev DB; verified by matching inodes in both containers. FINAL TAP: user `@jsbdm` tapped **Renta** on *Compra suelta* → `⏳ Procesando…` ack, store `status=answered` + `actual_category_id=61a89d79… (Renta)`, dev Actual tx `b1bb608c…` verified with category Renta via API read, then **reverted to null** (seed invariant: 2 uncategorized restored). **Live e2e CLOSED end-to-end.**

**Live e2e batch 2 (verify step 6 completion, same day)**: user tapped BOTH report-2 messages — *Nómina empresa* → **Transporte** (item 3, `c2f1d507…`) and *Compra suelta* → **Suscripciones** (item 4, `497d9e1f…`); both `applied` with `answered_by_name=jsbdm`, both verified in dev Actual via API (Transporte / Suscripciones), message UI showed "Respondido por jsbdm" + disabled buttons (live editMessageText confirmed). Follow-up delivery immediately after: `Movimientos sin categorizar detectados: 0`, report 3 row `sent: 0` — the exact drop §12A point 3 predicted (2 → 0 after all answered). Seed invariant restored afterward (both txs back to null → 2 uncategorized). NOTE: live second-tap-same-message ("ya respondido por X") NOT exercised live (user tapped distinct messages); it is proven by the dry-run replay (answer rows 3/5: `duplicate (already answered)`) + the guarded-UPDATE unit evidence from S1.

**Live e2e batch 3 (DEDICATED dev bot + dev group — env separation validation, 2026-09-20)**: user created a dedicated dev bot (`8451618156…`) + new dev group; `.env` updated (old prod creds left commented). Rationale: `getUpdates` is per-BOT, so one shared bot in dev+prod groups makes long-poll streams collide (409 / stale-offset cross-consumption); separate bots isolate the streams with ZERO code changes. Delivery (report 4): summary + 2 interactive messages, `sent=2, failed=0`, chat `-1004113118521`. Taps by `jordisf`: *Nómina empresa* → **Ocio y Restaurantes** (item 5, `f923f3f3…`), *Compra suelta* → **Supermercado y Alimentación** (item 6, `9c101059…`); both `applied`, both verified via API read-back on dev Actual. Follow-up delivery (report 5): `Movimientos sin categorizar detectados: 0`, `sent: 0`. Seed invariant restored (2 uncategorized). **GOTCHA (recorded)**: switching the dev token to a brand-new bot with the persisted `poll_offset` of the OLD bot (`545467812`) means the listener would NEVER see callback updates again (offset is a per-bot server counter; a stale-high offset is an EOF). Manually reset `kv poll_offset` → `0` before the e2e; if the PRODUCTION token is ever rotated, the same reset is required. **GOTCHA (recorded)**: reverting Actual categories via `api.updateTransaction(id, {category:null})` discards if the process `process.exit()`s — the mutation only flushes through the proper `actual.close(h)` lifecycle; the verified recipe (transcript) closes the handle before exiting.

**Duplicate-tap race (second tap on same message) — exercised on live batch-3 data (2026-09-20)**: a *sequential* second tap is impossible in Telegram (buttons vanish once the message is edited); the real case is the SIMULTANEOUS race — two humans tapping near-simultaneously, both callbacks arriving before either `editMessageText`. Executed via `replay-callback.js --item-ref=5 --ref=3` through the REAL `handleUpdate` (offline path, dev store): outcome `duplicate (already answered)` logged; winner row UNCHANGED (`status=answered, answered_by_name=jordisf, chosen_ref=4, category f923f3f3…`); synthetic answer row removed after verification. The Telegram toast ("⚠️ Ya respondido por X") cannot render for a synthetic callback id (HTTP 400, expected, harmless) — the decision and state logic is byte-for-byte the live code path. Closes verify-report WARNING #1.
